...
class VisualizeDataset:
    ...
    def save(self, plot_obj, fname=None, formats=('pdf', 'png'), ):
        if fname is None:
            ...
            fig_name = f'figure_{self.plot_number}'
            for format in formats:
                save_path = self.figures_dir / f'{fname}.{format}'
                plot_obj.savefig(save_path)
                print(f'Figure saved to {save_path}')
    ...
    def plot_dataset(self, data_table, columns, match='like', display='line', fname=None):
    ...
    def plot_binary_outliers(self, data_table, col, outlier_col, fname):
