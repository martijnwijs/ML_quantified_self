...
class FourierTransformation:
    ...
    def find_fft_transformation(self, data):
        ...
        # print(f"real_ampl.shape: {real_ampl.shape}")
    ...
    def abstract_frequency(self, data_table, columns, window_size, sampling_rate):
        ...
        # print(f"self.freqs.shape: {self.freqs.shape}")
        ...
        for col in columns:
            ...
            # note to self! Rolling window_size would be nicer and more logical! In older version windowsize is actually 41. (D + 1)
            data_table[col].rolling(window_size + 1).apply(self.find_fft_transformation)

            # Pad the missing rows with nans
            frequencies = np.pad(np.array(self.temp_list), ((window_size, 0), (0, 0)), 'constant',
